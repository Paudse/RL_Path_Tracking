...
# 6_reward-86.3
data = scipy.io.loadmat('6_reward-86.3')
cx2 = data['cx'][0]
cy2 = data['cy'][0]
cv2 = data['cv'][0]
sim_t2 = data['sim_t'][0]
...
plt.title('(a) Varied Speed', loc='left')


ax = plt.subplot2grid((2, 1), (1, 0), rowspan=1, colspan=1)
plt.plot(cx2, cy2, "-r", label="Ref")
plt.plot(sim_x2, sim_y2, "-g", label="PP+PPO")
plt.plot(sim_x_pp2, sim_y_pp2, "--b", label="PP")
plt.grid(True)
plt.axis("equal")
plt.xlabel("X (m)")
plt.ylabel("Y (m)")
...
plt.title('(b) Varied Corner', loc='left')
